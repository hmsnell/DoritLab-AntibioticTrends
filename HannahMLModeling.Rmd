...
```{r}
options(scipen = 999)
distbyclass <- ggplot(combinediqvia, aes(x = class, fill = "blue")) + 
                        geom_histogram(stat = "count") + 
                        labs(title = "Distribution of Antibiotic Classes in Dataset", x = "Class", y = "Row Count") + 
  theme(axis.text.x = element_text(angle = 90))
distbyclass
```
...
totalclaimcounts <- ggplot(totalclaims, aes(x = class, y = total, fill = "blue")) +
  geom_bar(stat = "identity") + 
  labs(x = "Class", y = "Total Claim Count", title = "Total Claim Count of Each Antibiotic Class") + 
  theme(axis.text.x = element_text(angle = 90))
totalclaimcounts
```
