...
DOUBLE CHECKING TOTAL PRESCRIPTION COUNT
```{r}
subyear <- AP_combinediqvia %>%
  filter(Year == "2016")

sum(subyear$total_claim_count)
```
...
pen$Region[pen$Region == "Undefined Region"] <- "*Other"

pen_line <- ggplot(pen, aes(x = Year, y = percapita_Enroll, group = Region, colour = Region)) +
  geom_line(size = 1.5) + 
  labs(x = "\nYear\n\n", 
       y = expression(paste(italic('Per capita\n'), ' Prescriptions\n')), 
       title = expression(paste('Annual\n', italic(' per capita\n'), ' Penicillin Prescriptions\n')), 
       subtitle = "2013 - 2017\n", 
...
pen_line + theme_economist() + 
  ...
  scale_color_discrete(breaks = c("Midwest",
                                  "Northeast",
                                  "South",
                                  "West",
                                  "*Other"))
...
top6_line <- ggplot(top6, aes(x = Year, y = percapita_Enroll, group = class, colour = class)) + 
  ...
  labs(x = "\nYear\n\n", 
       y = expression(paste(italic('Per capita\n'), ' Prescriptions\n')), 
       title = expression(paste('Annual', italic(' per capita'), ' Antibiotic Prescriptions')),
       subtitle = "2013 - 2017\n",
...
quet$Region[quet$Region == "Undefined Region"] <- "*Other"

quet_line <- ggplot(quet, aes(x = Year, y = percapita_Enroll, group = Region, colour = Region)) +
  geom_line(size = 1.75) + 
  labs(x = "\nYear\n\n", 
       y = expression(paste(italic('Per capita\n'), ' Prescriptions\n')), 
